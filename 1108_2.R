...
install.packages("DBI") #database를 사용하기 위한 package
library(rJava)
library(RJDBC)
library(DBI)
#순서대로 로딩
...
drv <- JDBC(driverClass = "com.mysql.jdbc.Driver",
           classPath = "C:/lecture2/mysql-connector-java-5.1.48-bin.jar") #classPath 공유폴더에서 받아온것

# R언어에서 Database 연결

conn <- dbConnect(drv, #접속하기 위해 드라이버를 들고 들어가야함
...
sql = "select btitle from book" #book table에서 btitle을 끌고 올거야
sql = "select * from book" #book table에서 전체를 끌고와라
