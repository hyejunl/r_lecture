...
summary(score_df2)
boxplot(score_df2) $stats #여기에서 극단치 최대 최소를 보고 구할 수 있다
box<-boxplot(score_df2)$stats

deter_value=c()
for (i in 2:length(score_df2)){
  deter_value[i]<-(quantile(score_df2[,i],0.75)-quantile(score_df2[,i],0.25))*1.5
  
  score_df2[,i][score_df2[,i]<box[2,i]-deter_value[i]]<-box[2,i]
  score_df2[,i][score_df2[,i]>box[4,i]+deter_value[i]]<-box[4,i]
